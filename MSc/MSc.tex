...
\title{Praca magisterska \\ Aplikacja do pomiarów szybkoœci transmisji danych w urz¹dzeniach z~systemem operacyjnym Android}
\author{Tomasz £akomy}
\date{}
...
\qquad Celem niniejszej pracy magisterskiej jest stworzenie systemu pomiarowego sk³adaj¹cego siê z aplikacji zaimplementowanej w jêzyku programowania Java na urz¹dzenia z systemem Android oraz aplikacji napisanych w jêzyku Python na komputer PC pe³ni¹cych funkcjê serwera retransmituj¹cego pakiety. Zadaniem tego systemu jest transmisja pakietów o zadanej d³ugoœci co okreœlony przez u¿ytkownika okres czasu oraz z wykorzystaniem zadanego protoko³u, nastêpnie retransmisja przes³anych pakietów przez serwer do aplikacji w celu dokonania pomiarów czasu transmisji oraz mocy sygna³u.

\qquad Aplikacja umo¿liwia transmisjê pakietów za pomoc¹ protoko³ów HTTP, TCP oraz UDP, a ponadto posiada szereg funkcjonalnoœci takich jak: pomiar pozycji terminala mobilnego, wyliczanie statystyk pomiaru, zapisywanie danych do pliku, mo¿liwoœæ pracy w tle, oraz zapisywanie zrzutów ekranu w pamiêci telefonu.

\subsection*{Abstract}
\qquad The goal of this Master's thesis is to create a measurement system which is composed of an application written in programming language Java developed for Android devices and applications written in Python which purpose is to act as a server for retransmitting previously sent packets. Created system is responsible for transmitting packets from mobile device of given length with a given interval, while using a protocol specified by the user. Afterwards the server retransmits the packets to the mobile phone in order for request time and signal strength measurements to be made.

\qquad The mobile phone application allows the user to send packets using HTTP, TCP or UDP protocols, along with functionalities such as measuring the mobile phone location, calculating measurement statistics, saving the data to file, background measurements and saving screenshots to phone data storage.
...
\newpage
\chapter{Wstêp}
\qquad We wspó³czesnym œwiecie dynamiczny rozwój technologii transmisji zarówno g³osu jak i danych ³¹czami radiowymi sprawia, ¿e telekomunikacja kablowa schodzi na dalszy plan. Coraz mniejsza czêœæ spo³eczeñstwa wykorzystuje w codziennej komunikacji z bliskimi telefony stacjonarne, obecnie w XXI wieku telefon komórkowy nie jest ju¿ symbolem statusu, lecz jego zakup sta³ siê niemal¿e koniecznoœci¹.

\qquad Gdy w latach 90-tych pojawi³y siê pierwsze sieci komórkowe GSM, ma³o kto spodziewa³ siê, ¿e w ci¹gu 20 lat u¿ytkownicy bêd¹ mieli mo¿liwoœæ transmisji na odleg³oœæ nie tylko g³osu, lecz tak¿e danych, zdjêæ, filmów, gier itd. Telefony komórkowe posiadaj¹ obecnie tak rozbudowane funkcje (g³ównie za spraw¹ tworzonych na nie aplikacji), ¿e zastêpuj¹ one z sukcesem komputery stacjonarne w takich zastosowaniach jak np. bankowoœc elektroniczna, czy zakupy przez Internet.

\qquad Ze wzglêdu na tak powszechny dostêp do szybkich transmisji za pomoc¹ ³¹cz radiowych w telefonii komórkowej, do celów telemetrycznych stosuje siê tak¿e radiokomunikacje. Wygoda u¿ytkowania oraz transportu terminala mobilnego sprawia, ¿e jest to doskona³e narzêdzie do przeprowadzania pomiarów jakoœci transmisji w sieciach bezprzewodowych. Spora gama dostêpnych w Polsce operatorów telefonii komórkowej sprawia, ¿e pomiary jakoœci transmisji zyskuj¹ na znaczeniu ze wzglêdu na liczbê stacji bazowych w zasiêgu terminala mobilnego, ró¿ne zasiêgi operatorów, rozmaite tryby transmisji oraz mo¿liwe zak³ócenia w kanale radiowym.

\qquad Celem niniejszej pracy jest stworzenie systemu pomiarowego, który sk³ada  siê z aplikacji napisanej na telefon z systemem operacyjnym Android oraz serwerów uruchamianych na komputerze PC. Zadaniem tego systemu jest nadawanie pakietów z terminala mobilnego za pomoc¹ trzech dostêpnych protoko³ów (HTTP, TCP oraz UDP), odbiór pakietu po stronie serwera oraz jego retransmisja do aplikacji w celu dokonania pomiaru czasu transmisji pakietu oraz mocy sygna³u.

\qquad Wynikiem ukoñczonej pracy jest aplikacja na urz¹dzenia z systemem Android zaimplementowana w jêzyku Java, któr¹ mo¿na z powodzeniem uruchomiæ i u¿ywaæ na zdecydowanej wiêkszoœci obecnie dostêpnych na rynku urz¹dzeñ dzia³aj¹cych pod kontrol¹ systemu Android, oraz aplikacje-serwery zaimplementowane w jêzyku Python, które mo¿na uruchomiæ na komputerach z systemami Windows, MacOS oraz Linux.

\qquad Ca³oœæ pracy sk³ada siê z piêciu rozdzia³ów, jej struktura jest nastêpuj¹ca:
\begin{itemize}
  \item Rozdzia³ pierwszy zawiera wstêp do pracy.
  \item Rozdzia³ drugi zawiera teoretyczny opis technologii transmisji zastosowanych w trakcie realizacji pracy. Przedstawiono opis technologii radiokomunikacji ruchomej zastosowanych w pracy, protoko³ów transmisji danych w sieci Internet, oraz charakterystykê tworzenia oprogramowania na urz¹dzenia z systemem Android oraz w jêzyku programowania Java. Opisano tak¿e aktualnie dostêpne na rynku przyk³adowe narzêdzia do pomiarów parametrów transmisji dla urz¹dzeñ z systemem 
Android.
  \item W rozdziale trzecim opisano praktyczn¹ realizacjê zaimplementowanego na potrzeby pracy oprogramowania. Opisano zarówno sposób implementacji serwerów jak i przedstawiono w szczegó³owy sposób detale implementacji poszczególnych funkcjonalnoœci aplikacji mobilnej
  \item Rozdzia³ czwarty zawiera przyk³adowe pomiary dokonane za pomoc¹ systemu bêd¹cego przedmiotem niniejszej pracy magisterskiej.
  \item Rozdzia³ pi¹ty zawiera podsumowanie pracy wraz z bibliografi¹, spisem rysunków i tabel.
\end{itemize}
...
\begin{table}[H]
    \begin{center}
        ...
        GSM 400 & 450,4 - 457,6
lub \newline 478,8 - 486 & 460,4 - 467,6
lub \newline 488,8 - 496 & 35 \\ \hline
...
\begin{table}[H]
    \begin{center}
        ...
        Wykorzystana metoda wielodostêpu - ³¹cze w dó³ & OFDMA \\ \hline
        Wykorzystana metoda wielodostêpu - ³¹cze w górê & SC-FDMA \\ \hline
        \end{tabular}
...
\qquad Do zalet programowania w jêzyku Python nale¿y zaliczyæ ogromn¹ iloœæ dostêpnych bibliotek, obszern¹ bibliotekê standardow¹, prostotê pisania i~czytania kodu oraz mo¿liwoœæ uruchamiania stworzonego w tym jêzyku oprogramowania na wielu ró¿nych platformach. Python jest jêzykiem interpretowanym, co oznacza, ¿e do uruchomienia programu napisanego w tym jêzyku nie jest wymagana kompilacja. Pozwala to na zwiêkszenie szybkoœci pracy nad oprogramowaniem, a~ró¿nice w prêdkoœci dzia³ania programów napisanych w Pythonie i~tych napisanych w jêzykach kompilowanych nie s¹ widoczne w wiêkszoœci popularnych zastosowañ.
...
\section{Aktualnie dostêpne przyk³adowe narzêdzia do pomiarów parametrów transmisji dla urz¹dzeñ z~systemem Android}

\subsection{Aplikacja RFBenchmark}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/rfbrenchmark_mainscreen.png}
    \caption{Ekran g³ówny aplikacji RFBenchmark}
    \label{fig:rfbrenchmark_mainscreen}
\end{figure}

\qquad Aplikacja RFBenchmark (przedstawiona na rysunku \ref{fig:rfbrenchmark_mainscreen}) jest komercyjnym systemem pomiaru jakoœci po³¹czenia z~sieciami komórkowymi. Pozwala ona u¿ytkownikowi na ocenienie jakoœci transmisji przy po³¹czeniu z~sieci¹ komórkow¹ na podstawie w³asnych pomiarów oraz na podstawie pomiarów w bazie danych aplikacji \cite{[17]}. Aplikacja ta jest dostêpna na urz¹dzenia z~systemem Android, iOS oraz wewn¹trz przegl¹darki internetowej.

\qquad Na rysunku \ref{fig:rfbrenchmark_mainscreen} s¹ widoczne naniesione na mape ró¿nokolorowe okrêgi. Ich barwa reprezentuje jakoœæ transmisji sieci komórkowej w danej pozycji na mapie. U¿ytkownik ma mo¿liwoœæ zmiany pozycji obserwowanej na mapie, a~ponadto w górnym panelu aplikacji znajduj¹ siê podstawowe informacje o~aktualnej sieci.

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/rfbenchmark_measurement.png}
    \caption{Pomiar prêdkoœci sieci w aplikacji RFBenchmark}
    \label{fig:rfbenchmark_measurement}
\end{figure}

\qquad Rysunek \ref{fig:rfbenchmark_measurement} przedstawia kolejn¹ funkcjonalnoœc aplikacji RFBenchmark, któr¹ jest mo¿liwoœc dokonywania w³asnych pomiarów jakoœci sieci. U¿ytkownik mo¿e zobaczyæ czas transmisji z~i~do serwera RFBenchmark, oraz przybli¿on¹ prêdkoœæ pobierania oraz wysy³ania plików w sieci.

\qquad W przeciwieñstwie do aplikacji bêd¹cej przedmiotem niniejszej pracy magisterskiej, u¿ytkownik nie ma mo¿liwoœci wyboru serwera z~którym prowadzona bêdzie transmisja, rozmiaru pakietu, czy protoko³u transmisji. Nie istnieje tak¿e mo¿liwoœc zapisu wyników do pliku, tylko jako publikacja na portalu spo³ecznoœciowym.

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/rfbenchmark_signalinfo.png}
    \caption{Widok informacji o~aktualnym ³¹czu radiowym i~sieci w aplikacji RFBenchmark}
    \label{fig:rfbenchmark_signalinfo}
\end{figure}

\qquad Aplikacja RFBenchmark udostêpnia tak¿e szczegó³we informacje na temat aktualnego ³¹cza radiowego z~sieci¹ komórkow¹ (rysunek \ref{fig:rfbenchmark_signalinfo}). Pozwalaj¹ one na podgl¹d m.in. aktualnej mocy sygna³u, informacji o~stacji bazowej oraz po³o¿enia terminala mobilnego.

\subsection{Aplikacja netmonitor}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/netmonitor_mainscreen.png}
    \caption{Ekran g³ówny aplikacji netmonitor}
    \label{fig:netmonitor_mainscreen}
\end{figure}

\qquad Aplikacja netmonitor, której ekran g³ówny przedstawia rysunek \ref{fig:netmonitor_mainscreen} jest zdecydowanie mniej zaawansowanym rozwi¹zaniem do pomiarów jakoœci transmisji w sieci od opisanej w poprzednim rozdziale aplikacji RFBenchmark. Na ekranie g³ównym u¿ytkownik ma mo¿liwoœæ zapoznania siê z~informacjami o~sieci komórkowej, z~któr¹ jest aktualnie po³¹czony oraz zapoznania siê z~wykresem zmiany si³y sygna³u w funkcji czasu. Dodatkowo aplikacja wyœwietla przybli¿ony adres, pod którym znajduje siê u¿ytkownik.

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/netmonitor_networks.png}
    \caption{Widok dostêpnych sieci w aplikacji netmonitor}
    \label{fig:netmonitor_networks}
\end{figure}

\qquad Wewn¹trz aplikacji netmonitor istnieje mo¿liwoœæ pomiaru mocy sygna³u sieci WiFi, które s¹ dostêpne w zasiêgu terminala komórkowego (rysunek \ref{fig:netmonitor_networks}). 

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/netmonitor_gps_position.png}
    \caption{Widok pozycji terminala mobilnego w aplikacji netmonitor}
    \label{fig:netmonitor_gps_position}
\end{figure}

\qquad Aplikacja umo¿liwia tak¿e pomiar aktualnej pozycji terminala mobilnego, wraz z~wyœwietleniem pomiaru jakoœci transmisji w danym miejscu. Jakoœæ ta jest wyœwietlana w postaci barwy zielonej, ¿ó³tej lub czerwonej, w zale¿noœci od tego jaka jest jakoœæ transmisji w danym po³o¿eniu. Nie istnieje mo¿liwoœæ uzyskania dok³adnych informacji na temat transmisji na podstawie której dokonywany jest pomiar.

\subsection{Aplikacja Ping Tools}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/ping_tools_mainscreen.png}
    \caption{Widok g³ówny aplikacji Ping Tools}
    \label{fig:ping_tools_mainscreen}
\end{figure}

\qquad Ping Tools jest aplikacj¹ której zadaniem s¹ pomiary transmisji z~terminala mobilnego do dowolnego serwera. Transmisja ta odbywa siê za pomoc¹ protoko³u HTTP, istnieje tak¿e mo¿liwoœæ ustawienia odstêpu czasu pomiêdzy kolejnymi pakietami wysy³anymi w sieci.

\qquad Ekran g³ówny aplikacji (rysunek \ref{fig:ping_tools_mainscreen}) przedstawia informacje na temat sieci WiFi, z~któr¹ po³¹czony jest u¿ytkownik. Wyœwietlane s¹ takie dane jak lokalne IP terminala mobilnego, bramka sieciowa, zewnêtrzne IP, si³a sygna³u WiFi oraz urz¹dzenia po³¹czone z~t¹ sam¹ sieci¹ (rysunek \ref{fig:ping_tools_connected_devices}).

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/ping_tools_connected_devices.png}
    \caption{Widok po³¹czonych z~lokaln¹ WiFi urz¹dzeñ w aplikacji Ping Tools}
    \label{fig:ping_tools_connected_devices}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/ping_tools_menu.png}
    \caption{Dostêpne opcje pomiaru w aplikacji Ping Tools}
    \label{fig:ping_tools_menu}
\end{figure}

\qquad Aplikacja Ping Tools umo¿liwia u¿ytkownikowi rozmaite opcje pomiaru poza pomiarem szybkoœci transmisji z~i~do serwera. Istnieje np. mo¿liwoœæ skanowania otwartych portów innego urz¹dzenia po³¹czonego z~Internetem, podgl¹d drogi, jakie pokonuj¹ pakiety w trakcie transmisji do serwera, czy skaner UPnP (ang. \textit{Universal Plug-and-Play}).

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/ping_tools_pingresult.png}
    \caption{Widok pomiaru czasu transmisji z~i~do serwera w aplikacji Ping Tools}
    \label{fig:ping_tools_pingresult}
\end{figure}

\qquad Rysunek \ref{fig:ping_tools_pingresult} przedstawia wynik pomiaru czasu transmisji do serwera firmy Google. U¿ytkownik ma mo¿liwoœæ wyboru liczby pakietów, a~po zakoñczeniu pomiaru - zapoznania siê ze statystykami pomiaru oraz udostêpnienia ich w innej aplikacji.

\subsection{Aplikacja Net Monitor}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/net_monitor_mainscreen.png}
    \caption{Widok g³ówny aplikacji Net Monitor}
    \label{fig:net_monitor_mainscreen}
\end{figure}

\qquad Ekran g³ówny aplikacji Net Monitor (która, mimo bardzo zbli¿onej nazwy do wczeœniej opisywanej aplikacji netmonitor, jest tworem innej firmy) zosta³ przedstawiony na rysunku \ref{fig:net_monitor_mainscreen}. Przedstawia on podstawowe informacje na temat aktualnego po³¹czenia z~sieci¹ komórkow¹ takie jak: nazwa operatora, aktualna sieæ, si³a sygna³u czy pozycja GPS. Dodatkowo istnieje tak¿e mo¿liwoœæ uzyskania informacji o~s¹siednich stacjach bazowych, je¿eli znajduj¹ siê one w zasiêgu transmisji telefonu.

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/net_monitor_measurement.png}
    \caption{Widok pomiaru ³¹cza radiowego w aplikacji Net Monitor}
    \label{fig:net_monitor_measurement}
\end{figure}

Rysunek \ref{fig:net_monitor_measurement} przedstawia widok pomiaru jakoœci transmisji telefonu z~stacj¹ bazow¹. Na wykresie przedstawione zosta³y wskaŸniki RSSI (ang. \textit{Received Signal Strength Indication}) dla sieci GSM, WCDMA oraz LTE. Dla sieci LTE dodatkowo przedstawione s¹ wskaŸniki RSRQ (ang. \textit{Reference Signal Received Quality}) oraz RSSNR (ang. \textit{Reference Signal Signal to Noise Ratio}). Pozwala to na analizê zmian jakoœci transmisji pomiêdzy terminalem mobilnym, a~stacj¹ bazow¹ w ci¹gu dnia.

\begin{figure}[H]
    \centering
\includegraphics[scale=0.135,natwidth=1080,natheight=1920]{screeny/net_monitor_sessions.png}
    \caption{Widok zapisanych sesji pomiarów w aplikacji Net Monitor}
    \label{fig:net_monitor_sessions}
\end{figure}

\qquad Aplikacja Net Monitor umo¿liwia tak¿e zapisywanie wewn¹trz aplikacji dokonywanych sesji pomiarowych, co przedstawia rysunek \ref{fig:net_monitor_sessions}. Ka¿da sesja ma swoj¹ nazwê, oraz zawiera informacjê o~czasie rozpoczêcia pomiaru, czasie trwania pomiaru, iloœci pakietów wysy³anych oraz rozk³ad iloœci wys³anych pakietów z~podzia³em na sieci GSM, WCDMA oraz LTE.
...
\begin{itemize}
\item Adresu IP serwera
\item Portu, na którym nas³uchuje uruchomiony uprzednio serwer
\item Liczby pakietów, która zostanie przes³ana w trakcie pomiaru
\item Rozmiaru (w bajtach) pojedyñczego przesy³anego pakietu
\item Czasu pomiêdzy przes³aniem kolejnego pakietu. Minimalny dopuszczalny czas wynosi 500ms, u¿ytkownik ma tak¿e mo¿liwoœæ wyboru pomiêdzy podaniem wartoœci w sekundach, minutach oraz milisekundach.
\item Protoko³u utransmisji - do wyboru s¹ protoko³y HTTP, TCP oraz UDP
\end{itemize}
...
\begin{itemize}
\item Aktualne po³o¿enie terminala mobilnego na mapie (za poœrednictwem us³ugi Google Maps). W zale¿noœci od szybkoœci aktualnej transmisji, po³o¿enie jest zaznaczane jaœniejszym b¹dŸ ciemniejszym okrêgiem. Pozwala to oceniæ jakoœæ transmisji w danym miejscu w przypadku przemieszczania siê w trakcie pomiaru.
...
\item Medianê czasu transmisji pakietu
...
\qquad [\textit{Protokó³ transmisji, aktualna sieæ, rozmiar pakietu, liczba pakietów, odstêp pomiêdzy pakietami, œredni czas transmisji pakietu, mediana czasu transmisji, minimalny czas transmisji, maksymalny czas transmisji, odchylenie æwiartkowe}] oraz tabela zawieraj¹ca czasy transmisji ka¿dego pakietu wraz z moc¹ sygna³u i dok³adnym po³o¿eniem.
...
\qquad W programie tym skorzystano z~biblioteki o~nazwie Flask (ang. \textit{fiolka}), która w prosty sposób pozwala na realizacje obs³ugi zapytañ HTTP. Wysoki stopieñ abstrakcji interfejsu programisty udostêpnionego przez bibliotekê Flask sprawia, ¿e posiadaj¹c umiejêtnoœci programistyczne mo¿na implementowaæ zarówno proste jak i~bardziej skomplikowane aplikacje - serwery przy u¿yciu niewielkiej iloœci linii kodu.
...
\qquad Wysy³aj¹c pakiety TCP konieczne jest okreœlenie rozmiaru pakietu w bajtach. Program oczekuje, ¿e wœród przesy³anych danych pojawi siê sekwencja \textit{PACKET\_SIZE:N} (ang. \textit{Rozmiar pakietu}), gdzie N jest rozmiarem pakietu danych. Nastêpnie program (tak d³ugo, jak wysy³ane s¹ dane) odbiera pakiety o~zadanej d³ugoœci i~odsy³a je z~powrotem do terminala, który je nada³. W momencie, w którym terminal mobilny przestaje wysy³aæ dane, serwer pozostaje w czuwaniu i~czeka na kolejne pakiety do transmisji.
...
\qquad Aplikacja na smartfon z~systemem Android zosta³a przygotowana w œrodowisku Android Studio w wersji 1.3.1 i~zaimplementowana w jêzyku programowania Java. Aplikacja zosta³a napisana zak³adaj¹c wsparcie wersji systemu operacyjnego Android pocz¹wszy od 4.0.0, co w praktyce oznacza, ¿e ponad 94\% telefonów z~systemem Android jest w stanie j¹ poprawnie uruchomiæ i przeprowadzaæ pomiary. Dane te potwierdza rysunek \ref{fig:android_device_support}.
...
\item Rozmiaru (w bajtach) pojedyñczego przesy³anego pakietu
\item Czasu pomiêdzy przes³aniem kolejnego pakietu. Minimalny dopuszczalny czas wynosi 500ms, u¿ytkownik ma tak¿e mo¿liwoœæ wyboru pomiêdzy podaniem wartoœci w sekundach, minutach oraz milisekundach.
\item Protoko³u transmisji - do wyboru s¹ protoko³y HTTP, TCP oraz UDP
\end{enumerate}
...
\begin{figure}[H]
    \centering
\includegraphics[scale=0.18,natwidth=1920,natheight=1080]{measurement_screen.jpg}
    \caption{Ekran pomiaru}
    \label{fig:measurement_screen_android}
\end{figure}

\qquad Pod map¹ znajduj¹ siê informacje na temat bie¿¹cego pomiaru takie jak:
...
\qquad Aplikacja \textit{Android Ping} posiada funkcjonalnoœæ pomiaru aktualnej pozycji terminala mobilnego, wyœwietlania jej na mapie, oraz zapisywania pozycji do pliku. W tym celu skorzystano z~us³ugi udostêpnianej przez twórców Androida jak¹ jest Google Maps \cite{[15]}. 
...
\qquad Po po³¹czeniu siê z~us³ug¹ nawigacji nale¿y zdefiniowaæ funkcje przedstawione na rysunku \ref{fig:gps_listener}, które zostan¹ wykonane w przypadku np. zmiany pozycji terminala mobilnego. Najbardziej istotn¹ z~nich jest \textit{onLocationChanged}, która wykonywana jest za ka¿dym razem, gdy us³uga nawigacji Google odczyta now¹ pozycjê urz¹dzenia.
...
\qquad \textit{AndroidPingData\lbrack AKTUALNY\_CZAS\rbrack.csv}
...
\begin{figure}[H]
    \centering
\includegraphics[scale=0.5,natwidth=1039,natheight=487]{pomiary/edge_excel.png}
...
\qquad Poni¿sze wyniki przedstawiaj¹ przyk³adowe pomiary dokonane w œrodowisku miejskim, terminal mobilny pozostawa³ nieruchomy przez ca³y czas pomiaru. Nadano 50 pakietów o~rozmiarze 1024 bajtów za pomoc¹ protoko³u HTTP, z~odstêpem czasu 2s pomiêdzy kolejnymi pakietami.

\qquad Po zakoñczeniu ka¿dego z pomiarów wykonano zrzuty ekranu w celu zobrazowania widoku statystyk dotycz¹cych pomiaru w aplikacji, oraz wykresu przedstawiaj¹cego zmianê czasu transmisji pakietu oraz mocy sygna³u w czasie. Dodatkowo zaimportowano automatycznie generowane przez aplikacjê wyniki w pliku .csv do programu Microsoft Excel w celu wykazania, ¿e aplikacja umo¿liwia dalsz¹ analzê danych za pomoc¹ narzêdzi na komputer PC.

\section{Pomiar nieruchomego terminala mobilnego (nadanie 50 pakietów, sieæ LTE)}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_lte_measurement.png}
    \caption{Widok aplikacji po zakoñczonym pomiarze - nadanie 50 pakietów, sieæ LTE}
    \label{fig:stationary_lte_http}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_lte_results.png}
    \caption{Widok wykresu wyników pomiaru w aplikacji - nadanie 50 pakietów, sieæ LTE}
    \label{fig:stationary_lte_http_results}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.5,natwidth=1048,natheight=486]{pomiary/lte_excel.png}
    \caption{Wyniki pomiaru nieruchomego terminala mobilnego - nadanie 50 pakietów, sieæ LTE, zapisane pomiary wyœwietlone za pomoc¹ programu Microsoft Excel}
    \label{fig:stationary_lte_http_excel}
\end{figure}

\section{Pomiar nieruchomego terminala mobilnego (nadanie 50 pakietów, sieæ WCDMA)}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_wcdma_measurement.png}
    \caption{Widok aplikacji po zakoñczonym pomiarze - nadanie 50 pakietów, sieæ WCDMA}
    \label{fig:stationary_wcdma_http}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_wcdma_results.png}
    \caption{Widok wykresu wyników pomiaru w aplikacji - nadanie 50 pakietów, sieæ WCDMA}
    \label{fig:stationary__wcdma_results}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.5,natwidth=1042,natheight=486]{pomiary/wcdma_excel.png}
    \caption{Wyniki pomiaru nieruchomego terminala mobilnego - nadanie 50 pakietów, sieæ WCDMA zapisane pomiary wyœwietlone za pomoc¹ programu Microsoft Excel}
    \label{fig:stationary_wcdma_http_excel}
\end{figure}

\section{Pomiar nieruchomego terminala mobilnego (nadanie 50 pakietów, sieæ EDGE)}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_edge_measurement.png}
    \caption{Widok aplikacji po zakoñczonym pomiarze - nadanie 50 pakietów, sieæ EDGE}
    \label{fig:stationary_wcdma_http}
\end{figure}

\begin{figure}[H]
    \centering
\includegraphics[scale=0.2,natwidth=1920,natheight=1080]{pomiary/app_http_edge_results.png}
    \caption{Widok wykresu wyników pomiaru w aplikacji - nadanie 50 pakietów, sieæ EDGE}
    \label{fig:stationary__wcdma_results}
\end{figure}
...
\begin{figure}[H]
    \centering
\includegraphics[scale=0.5,natwidth=1039,natheight=487]{pomiary/edge_excel.png}
    \caption{Wyniki pomiaru nieruchomego terminala mobilnego - nadanie 50 pakietów, sieæ EDGE, zapisane pomiary wyœwietlone za pomoc¹ programu Microsoft Excel}
    \label{fig:stationary_wcdma_http_excel}
\end{figure}

\newpage
\chapter{Podsumowanie}
\qquad Celem pracy by³o zaimplementowanie systemu pomiarowego do weryfikacji szybkoœci transmisji danych w sieciach komórkowych zgodnie z wczeœniej opisanymi za³o¿eniami projektowymi. Nale¿a³o przygotowaæ zarówno aplikacjê na urz¹dzenia dzia³aj¹ce pod kontrol¹ systemu operacyjnego Android, która pozwala na transmisjê pakietów za pomoc¹ protoko³ów HTTP, TCP oraz UDP oraz serwery, które s¹ w stanie odbieraæ takie pakiety i retransmitowaæ je z powrotem do aplikacji mobilnej.

\qquad W trakcie tworzenia pracy uda³o siê zrealizowaæ wszystkie za³o¿enia projektowe, powsta³a aplikacja posiada szereg u¿ytecznych dla celów pomiarowych oraz analizy zgromadzonych danych funkcjonalnoœci. Dziêki zapisywaniu danych do pliku u¿ytkownik aplikacji mo¿e analizowaæ wykonane wczeœniej pomiary przy u¿yciu zewnêtrznego oprogramowania (takiego jak program Microsoft Excel, czy inne narzêdzia do analizy statystycznej). 

\qquad System pomiarowy zosta³ opracowany w taki sposób, ¿eby mo¿na by³o go z ³atwoœci¹ rozbudowaæ. Mo¿na przyk³adowo dodaæ mo¿liwoœæ transmisji z wykorzystaniem innych protoko³ów bez naruszania istniej¹cej implementacji transmisji danych. Dodatkowo aplikacja mo¿e w przysz³oœci liczyæ inne statystyki w trakcie pomiaru i zapisywaæ je do pliku (np. odchylenie standardowe) lub wysy³aæ powiadomienia o zakoñczonym pomiarze wraz z wynikami poczt¹ elektroniczn¹.
...
\begin{thebibliography}{10}
    ...
    \bibitem{[17]} ród³o internetowe: \textit{Strona internetowa aplikacji RFBenchmark} \\ http://www.rfbenchmark.eu/
